...
class LeNet5(nn.Module):
    def __init__(self, num_classes=10, n_input_channels=1, n_input_padding=2):
        ...
        # FashionMNIST and MNIST with shape (1, 28, 28): n_input_channels=1, n_input_padding=2
        self.layer1 = nn.Sequential(
            nn.Conv2d(n_input_channels, 6, kernel_size=3, stride=1, padding=n_input_padding),
            nn.BatchNorm2d(6),
            nn.ReLU(),
            nn.AvgPool2d(kernel_size=2, stride=2, padding=1))  # apply average pooling instead
        self.layer2 = nn.Sequential(
            nn.Conv2d(6, 16, kernel_size=3, stride=1, padding=0),
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.AvgPool2d(kernel_size=2, stride=2))
        self.fc0 = nn.Linear(576, 200)
        # self.fc0 = nn.Linear(256, 120)
        self.relu = nn.ReLU()
        self.fc1 = nn.Linear(200, 90)
        self.relu1 = nn.ReLU()
