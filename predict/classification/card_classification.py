...
class Diffusion(object):
    ...
    def compute_val_before_softmax(self, gen_y_0_val):
        """
    ...
    def p_sample_loop_only_y_0(self, model, x, y_0_hat, y_T_mean, n_steps,
        ...
        if not self.args.train_guidance_only:
            ...
            for epoch in range(start_epoch, self.config.training.n_epochs):
                ...
                for i, feature_label_set in enumerate(train_loader):
                    ...
                    if config.model.arch in ["simple", "linear"]:
                ...
                if epoch % self.config.training.validation_freq == 0 \
                    ...
                    for test_batch_idx, (images, target) in enumerate(test_loader):
                        ...
                        if config.model.arch == "simple" or config.model.arch == "linear":
                            images = torch.flatten(images, 1)
                        # true_target.append(list(target))
                        ...
                        with torch.no_grad():
                            ...
                            # predict.append(label_t_0.detach().cpu())
                            fp, tp, fn, tn = recall_pre_f1(label_t_0.detach().cpu(), target.cpu())
                    ...
                    reacll = tp_all / (tp_all + fn_all)
                    precision = tp_all / (tp_all + fp_all)
                    f1 = 2 * reacll * precision / (reacll + precision)
                    torch.save(states, os.path.join(self.args.log_path, "ckpt_best_epoch{}.pth".format(epoch)))
                    if precision > max_precision:
                        logging.info("Update best precision at Epoch {}.".format(epoch))
                        torch.save(states, os.path.join(self.args.log_path, "ckpt_best.pth"))
                    max_precision = max(max_precision, precision)
                    if not tb_logger is None:
                        tb_logger.add_scalar('precision', precision, global_step=step)
                    logging.info(
                        (
                                ...
                                f"Max precision: {max_precision:.6f}   "
                        )
                    )
            ...
            if self.config.model.ema:
                states.append(ema_helper.state_dict())
            torch.save(states, os.path.join(self.args.log_path, "ckpt.pth"))
            # save auxiliary model after training is finished  不运行
            if config.diffusion.apply_aux_cls and config.diffusion.aux_cls.joint_train:
                aux_states = [
                    self.cond_pred_model.state_dict(),
                    aux_optimizer.state_dict(),
...
#将测试集中的调整缩放温度参数  调整到测试集中
            ...
            model = model.to(self.device)
            model.load_state_dict(states[0], strict=True)  #将训练好的最佳的一次权重传入到模型中
            ...
            if args.tune_T:
                ...
                logging.info("Begin generating {} samples for tuning temperature scaling parameter...".format(n_tune_T_samples))
                for idx, feature_label_set in tqdm(enumerate(train_loader)):  # test_loader would give oracle hyperparameter
                    ...
                    y_0_sample_batch = p_sample_loop_only_y_0(model, x_tile, y_0_hat_tile, y_T_mean_tile,self.num_timesteps,self.alphas,self.one_minus_alphas_bar_sqrt)  # .cpu().numpy()
                    ...
                    if args.sanity_check:
                        if idx > 2:
                            break
                # logging.info("len(y_0_one_sample_all)：{}，y_0_one_sample_all[0].shape：{}".format(len(y_0_one_sample_all),y_0_one_sample_all[0].shape))
            ...
            states_T = [self.tuned_scale_T,]
            torch.save(states_T, os.path.join(self.args.log_path, "tuned_scale_T.pth"))
    ...
    def test_image_task(self):
        ...
        def compute_and_store_cls_metrics(config, y_labels_batch, generated_y, batch_size, num_t):
            ...
            if current_t == config.testing.metrics_t:
                ...
                if len(label_probs_by_batch_list[current_t]) == 0:
                    ...
                    ttest_pvalues_batch.append(ttest_pvalues)
                else:
        ...
        def compute_quantile_metrics(config, CI_all_classes, majority_voted_class, true_y_label):
            ...
            for c in range(config.data.num_classes):
                ...
                correct.append(CI_w_cor_class_c.shape[0])
                incorrect.append(CI_w_incor_class_c.shape[0])
            ...
            recall = int(correct[1])/(int(correct[1])+int(incorrect[1]))
            precision = int(correct[1])/(int(correct[1])+int(incorrect[0]))
            accuracy = (int(correct[0])+int(correct[1]))/(int(correct[0])+int(correct[1])+int(incorrect[0])+int(incorrect[1]))
        ...
        split = args.split
        log_path = os.path.join(self.args.log_path)
        dataset_object, test_dataset = get_test(args, config)
        #下边这个代码是将图片名称按照数值顺序排列
        test_dataset.samples = sorted(test_dataset.samples, key=lambda x: numeric_sort(os.path.basename(x[0])))
        test_loader = data.DataLoader(
            test_dataset,
            batch_size=config.testing.batch_size,
            shuffle=False,
        ...
        states_1 = torch.load(os.path.join(log_path, "tuned_scale_T.pth"), map_location=self.device)
        self.tuned_scale_T = states_1[0]
        logging.info(("\nThe parameter to adjust the scaling temperature obtained in the training part is：{:.4f}.\n").format(self.tuned_scale_T))
        ...
        results = []
        results.append(['majority_vote_by_batch_list', 'all_class_probs_by_batch_list', 'label_probs_by_batch_list',
                        'label_mean_probs_by_batch_list', 'CI_by_batch_list', 'ttest_pvalues_batch'])
        majority_vote_by_batch_list_all = majority_vote_by_batch_list[0].ravel().tolist()
        ttest_pvalues_batch_all = [num for sublist in ttest_pvalues_batch for num in sublist.flatten()]
        for i in range(len(majority_vote_by_batch_list[0])):
            results.append([majority_vote_by_batch_list_all[i],
                            list(all_class_probs_by_batch_list[0][i]),
                            list(label_probs_by_batch_list[0][i]),
                            list(label_mean_probs_by_batch_list[0][i]),
                            (CI_by_batch_list[0][i][1] - CI_by_batch_list[0][i][0]).squeeze().tolist(),
                            ttest_pvalues_batch_all[i].tolist()])

        file = open('output/output.bedpe', "w")
        for row in results:
            for item in row:
                file.write(str(item) + "\t")
            file.write("\n")
        ...
        majority_vote_accuracy_by_class, CI_width_correct_pred_by_class, CI_width_incorrect_pred_by_class = compute_quantile_metrics(
